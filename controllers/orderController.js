  ...
  try {
    ...
    if (order.amount !== order.totalAmount) {
      const discountPercentage = parseInt(
        ((req.body.totalAmount - req.body.amount) / req.body.totalAmount) * 100
      );
      coupon = await stripe.coupons.create({
        duration: "repeating",
        name: `${discountPercentage}% off on items`,
    ...
    if (coupon) {
      await User.findByIdAndUpdate(
        req.body.userId,
        { $pull: { coupon: { _id: req.body.coupon } } },
        { new: true }
      );
    }
...
const getDiscount = async (req, res) => {
  try {
    ...
    res.json({
      success: false,
      message: "error occured while getting coupon code",
    });
  }
};
export {
